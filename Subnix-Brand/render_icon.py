"""
Renders the Subnix app mark (Mark D — four cells) at any size.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "--macos":
        ...
        ios.convert("RGB").save("Subnix-iOS-1024.png")

        # Viewing preview: rounded, so you see what users actually see.
        prev = draw_mark(512, HONEY, CREAM, INK, rounded=True)
        prev.save("Subnix-preview-rounded.png")

        # Dark colorway preview (alt).
        darkp = draw_mark(512, INK, CREAM, HONEY, rounded=True)
        darkp.save("Subnix-preview-dark.png")

        print("rendered: Subnix-iOS-1024.png, Subnix-preview-rounded.png, Subnix-preview-dark.png")
