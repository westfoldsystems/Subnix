                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            VStack(alignment: .leading, spacing: 2) {
                                if host.ip == scanner.selfIP || host.isGateway {
                                    ...
                                    .font(.caption2).foregroundStyle(.subnixAccent)
                                }
                                if let bonjour = host.bonjourName {
                                    Text(bonjour).font(.caption2.weight(.medium)).foregroundStyle(.subnixInk)
                                }
                                if host.hostname != nil {
                                    Text(host.ip).font(.caption2).foregroundStyle(.subnixMuted)
                                }
                                if let hint = host.deviceHint {
                                    Text(hint).font(.caption2).foregroundStyle(.subnixMuted)
                                }
                                ...
                                if let tls = host.tlsName {
                                    Text("cert: \(tls)").font(.caption2).foregroundStyle(.subnixMuted)
                                }
                                if let vendor = host.vendor {
                                    Text(vendor).font(.caption2).foregroundStyle(.subnixMuted)
                                }
                            }
